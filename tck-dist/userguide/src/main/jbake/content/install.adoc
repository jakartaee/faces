...
.  Install the build tool Apache Maven {MavenVersion} that will be used to run the TCK, 
if it is not already installed, for running the tests. 
...
.  Install the {TechnologyShortName} TCK {TechnologyVersion} software.
  a.  Copy or download the {TechnologyShortName} TCK software to your
      ...
      Jakarta EE web site {SpecificationURL}.

...
web application to define an alternate suffix forJakarta Server Pagespages containing
...
==== Servlet andJakarta Server PagesAPI Classes (jakarta.servlet.*)
...
====Jakarta Server PagesStandard Tag Library (JSTL) API Classes (jakarta.servlet.jsp.jstl.*)
...
====Jakarta Server PagesStandard Tag Library (JSTL) Implementation Classes
...
JSP custom action that corresponds to a Jakarta Faces _UIComponent_.[P1-end] See
...
Faces depends on versionJakarta Server Pages2.1 or later,
and theJakarta Server Pagestags in Faces expose properties that leverage concepts
specific to that release of JSP. Importantly, most FacesJakarta Server Pagestag
...
Faces component libraries that expose themselves asJakarta Server Pagestags, the
existing classes relating toJakarta Server Pageshave been deprecated and new ones

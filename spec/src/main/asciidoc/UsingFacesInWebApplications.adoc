...
* _jakarta.faces.ADDITIONAL_HTML_EVENT_NAMES_ --
...
* _jakarta.faces.ENABLE_CSP_NONCE_ --
If this param is set, and calling _toLowerCase().equals("true")_ on a
_String_ representation of its value returns _true_, the runtime must
generate a Content Security Policy (CSP) nonce value for the current request.
The generated nonce can be obtained via the _ResourceHandler.getCurrentNonce()_
method and is intended to be used as the value of the _nonce_ attribute on
rendered _<script>_ elements and as part of the _Content-Security-Policy_
response header. See the javadocs for the constant
_jakarta.faces.application.ResourceHandler.ENABLE_CSP_NONCE_ and the method
_jakarta.faces.application.ResourceHandler.getCurrentNonce()_ for the complete
specification of this feature.

* _jakarta.faces.EXCEPTION_TYPES_TO_IGNORE_IN_LOGGING_ --
Comma-delimited list of fully qualified exception class names to ignore in logging.
Implementations and component libraries should consult this when determining whether
to log a given exception, and should ignore exceptions that are instances of the
specified types.
...
rendered when an error is generated.
...
a Facelets VDL view hits the _ViewHandler_ for page compilation.
...
regardless of their extension. See the
...
Therefore specifying full state saving view IDs via this context parameter will not be an option anymore.
...
*Deprecated since 4.1:*
Full state saving will be removed in favor of partial state saving in order to keep the spec simple.
Therefore disabling partial state saving via this context parameter will not be an option anymore.
...
application configuration resources have been processed. The runtime must
create and make available a _FacesContext_ instance during this event processing
as specified in <<Per-RequestStateInformation.adoc#a3235,FacesContext Availability>>.
...
. Publish the
_jakarta.faces.event.PreDestroyApplicationEvent_. The runtime must
create and make available a _FacesContext_ instance during this event processing
as specified in <<Per-RequestStateInformation.adoc#a3235,FacesContext Availability>>.
...
* _/faces-config/factory/search-expression-context-factory_ —This
element contains the fully qualified class name of the concrete
...
[[a6598]]
==== Requirements for discovery of classes with annotations
...
* The annotations listed in <<a6596>> are CDI stereotypes.
This means they are bean defining annotations, and classes annotated with them are discovered during CDI bean discovery even when `bean-discovery-mode` is `annotated`.
The implementation must use a CDI extension or a CDI build compatible extension to observe these annotated types during deployment and register them with the runtime.

* Library JARs containing classes with Faces configuration annotations must be CDI bean archives, i.e. they must contain a `META-INF/beans.xml` file.

* In order to exclude classes from annotation discovery, the application can use the `<exclude>` element in `beans.xml` or the `@jakarta.enterprise.inject.Vetoed` annotation on the class.

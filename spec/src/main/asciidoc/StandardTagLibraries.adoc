...
** If such a facet already exists and it is the
component associated with this tag, take no additional
action.

** If such a facet already exists and it is another
component, then, unless that facet is an automatically created
_UIPanel_ component, create one, make it the facet under the
specified facet name, and move the previously existing facet into
it as a child. Add the component associated with this tag as a
child of that _UIPanel_.
...
A _Facet_ is a subordinate _UIComponent_ that
...
in a page using the _<f:facet>_ tag. When the facet tag has exactly
one child _UIComponent_ tag, that child becomes the facet. When it has
more than one child _UIComponent_ tag, a _UIPanel_ component is
automatically created as the facet and the children are placed in
it <<Footnotes.adoc#a9099,^[10]^>>. When it has no child _UIComponent_
tag but does declare pass through attributes, a _UIPanel_ component is
automatically created as the facet as well. For example:
...
<h:dataTable ...>
  ...
  <h:column>
    <f:facet name="header" pt:styleClass="icon-email" />

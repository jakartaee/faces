...
This chapter describes theJakarta Server Pagessupport
required by Jakarta Server Faces. ThisJakarta Server Pagessupport is enabled by providing
...
provided by a Jakarta Faces implementation may be mixed with standardJakarta Server Pagesactions
and custom actions from other libraries, as well as template text for
layout, in the sameJakarta Server Pagespage.
...
ForJakarta Server Pagesversion 2.0 and onward, the file
...
content <<Footnotes.adoc#a9096,13>>. When running in aJakarta Server Pages1.2 environment,
JSP authors must give theirJakarta Server Pagespages that contain Jakarta Faces content a
filename ending in “_.jsp_”.

[[a4415]]
=== UIComponent Custom Actions

AJakarta Server Pagescustom action (aka custom tag or tag)
...
_RenderKit_ implementors will generally provide aJakarta Server Pagestag library which
...
=== Using UIComponent Custom Actions inJakarta Server PagesPages

The following subsections define how a page
author utilizes the custom actions provided by the _RenderKit_
implementor in theJakarta Server Pagespages that create the user interface of a
...
placed at any desired position in aJakarta Server Pagespage that contains the _taglib_
directive for the corresponding tag library, subject to the following
restrictions:

* When using a singleJakarta Server Pagespage to create the
...
use of a UIComponent custom action in aJakarta Server Pagespage. In this scenario:
...
actions is processed by theJakarta Server Pagescontainer, in the usual way. Therefore,
...
encountered during the processing of aJakarta Server Pagespage, the custom action
...
with eachJakarta Server Pagespage. For example, assume that a _UIForm_ component (whose
...
in aJakarta Server Pagespage using the _<f:facet>_ custom action. Each facet action
...
[[a4536]]
==== Interoperability withJakarta Server PagesTemplate Text and Other Tag Libraries

It is permissible to use other tag libraries,
such as the Jakarta Standard Tag Library (Jakarta Tags) in the sameJakarta Server Pagespage with
...
will evaluate. Please see section JSP.C.1 in theJakarta Server Pages2.1 specification
for details.
...
for tags that expose Faces components to theJakarta Server Pagespage. This section
...
delimiter, _#{}_ , to prevent theJakarta Server Pagescontainer from prematurely
...
* TheJakarta Server PagesContainer will hand the tag setter a
...
using #\{} as the delimiters for expressions that appear in aJakarta Server Pagespage
...
. thatJakarta Server Pagespages using the standard h: and f:
tags must work without change

. thatJakarta Server Pagespages using custom faces taglibs
...
element in aJakarta Server Pages1.2 (and earlier) TLD, as well as the _version_ element
in aJakarta Server Pages2.0 (and later) TLD. TheJakarta Server Pagescontainer must examine the
...
1.2 or later taglib and theJakarta Server Pagescontainer is aware of #\{} expressions.
...
or _tagdependent_ as described in theJakarta Server Pagesspecification. This section
...
the page author (and impossible for theJakarta Server Pagespage compiler) to enforce
this restriction.
...
*Jakarta Server Pagespage fragments included via the standard
...
Note, to avoid confusion betweenJakarta Server Pagesand

...
* Call _initView()_ on the _ViewHandler_ before invoking any
_PhaseListener.beforePhase()_ callbacks. This 
...
components, must implement _PartialStateHolder_ manually. The helper class
...
PartialStateHolder>>_ for details.
...
implement _PartialStateHolder_ correctly.
...
A _resourceName_ may carry a query string, introduced by a ’ _?_ ’
character. The query string is not part of the _<resourceIdentifier>_. The
rules above apply to the _resourceName_ without its query string, and it is
the _resourceName_ without its query string that identifies the resource, both
when resolving it and when recording that it has been rendered, as specified in
_ResourceHandler.createResource()_, _ResourceHandler.markResourceRendered()_
and __ResourceHandler.isResourceRendered()__.

The query string, without its introducing ’ _?_ ’ character, is appended
as one or more further name=value pairs to the resource metadata in the value
returned by __Resource.getRequestPath()__, separated from any pair preceding it
by a ’ _&_ ’ character. If there is no such pair, the query string is the
whole of the resource metadata. It must not use any of the parameter names that
the run-time itself uses there, namely ’ _ln_ ’, ’ _loc_ ’ and
’ _con_ ’; the value returned by __Resource.getRequestPath()__ for a
query string that does is unspecified.
...
public class ScriptRenderer extends Renderer<UIOutput>

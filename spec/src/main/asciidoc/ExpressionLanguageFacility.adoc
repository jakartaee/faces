...
image:ELResolverChain.svg[ELResolver chain]
...
The _resourceName_ may carry a query string, introduced by a ’ _?_ ’
character, as specified in <<RequestProcessingLifecycle.adoc#a758, Resource
Identifiers>>. The colon characters counted below are the ones preceding the
first ’ _?_ ’ character in _property_, so a query string may itself contain a
colon character. The query string stays with the _resourceName_ that is passed
to __ResourceHandler.createResource()__, which is what removes it from the
resource identifier and appends it to the value returned by
__Resource.getRequestPath()__.
...
. _jakarta.el.OptionalELResolver_
. _jakarta.el.RecordELResolver_
...
stored in the request, session, application, or import scopes by name.
...
Use the argument property as the key in a
call to facesContext.getELContext().getImportHandler().resolveClass(). If this returns
an instance of Class, call setPropertyResolved(true) on the argument ELContext and
return the value wrapped in a new instance of jakarta.el.ELClass.
...
kinds of objects into other objects using _@Inject_.

.Implicit Objects

The non-environment-specific implicit objects listed in
<<a2832,Implicit Objects for Programmatic Access>> must be produced as CDI beans,
making them available for _@Inject_ injection. This includes:

- _jakarta.faces.component.UIComponent_ (named _component_)
- _jakarta.faces.component.UIViewRoot_ (named _view_)
- _jakarta.faces.context.ExternalContext_ (named _externalContext_)
- _jakarta.faces.context.FacesContext_ (named _facesContext_)
- _jakarta.faces.context.Flash_ (named _flash_)
- _jakarta.faces.application.ResourceHandler_ (named _resource_)
- _jakarta.faces.flow.Flow_ (named _flow_)

Note that injection of environment-specific types is provided by the Jakarta Servlet
specification's built-in CDI beans, not by Jakarta Faces. The corresponding Faces
implicit objects are exposed only as named EL beans, not for typed _@Inject_ injection:

- _jakarta.servlet.http.HttpServletRequest_ (named _request_)
- _jakarta.servlet.http.HttpSession_ (named _session_)
- _jakarta.servlet.ServletContext_ (named _application_)
...
.Faces Expression Language Values

The _@jakarta.faces.annotation.ManagedProperty_ qualifier may be combined with
_@Inject_ on a field to inject the value of a Faces Expression Language expression.
The expression is evaluated lazily, on every access, against the current
_FacesContext_. The declared type of the injected field must be compatible with
the type of the resolved expression value.
...
The Implicit Objects for Facelets and Programmatic Access must be produced and named using CDI,
with the exception of environment-specific types which must only be named for EL resolution.

[[a2830]]
===== Implicit Objects for Facelets and Programmatic Access

The following Implicit Objects for Facelets and Programmatic Access must be produced and named
using CDI (making them available for both _@Inject_ and EL resolution), except for
environment-specific types _request_, _session_, and _application_, which must only be named
(using _@Named_) for EL resolution. The naming for environment-specific types can be accomplished
using either a CDI producer or an implicit EL resolver. This separation maintains proper
specification boundaries, as the CDI specification mandates that servlet containers provide
built-in beans for _@Inject_ support of _HttpServletRequest_, _HttpSession_, and _ServletContext_.
...
The CDI conversation scope is active during all Servlet requests.
When a long-running conversation is associated with the current Servlet request,
it may be propagated to any subsequent Servlet request via use of a request parameter named `cid`
containing the unique identifier of the conversation.
In this case, the application must manage this request parameter.

However, if the current Servlet request is a Faces request,
and the conversation is in long-running state,
it is propagated automatically according to the following rules:

* The long-running conversation context associated with a request that renders a Faces view
is automatically propagated to any Faces request (Faces form submission)
that originates from that rendered page.

* The long-running conversation context associated with a request that results in a Faces redirect
(a redirect resulting from a navigation rule or a _NavigationHandler_)
is automatically propagated to the resulting non-Faces request,
and to any other subsequent request to the same URL.

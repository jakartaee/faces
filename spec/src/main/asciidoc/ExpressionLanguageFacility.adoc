...
The Implicit Objects for Facelets and Programmatic Access must be resolved using CDI, with the
exception of environment-specific types which must be resolved via implicit EL resolvers.

[[a2830]]
===== Implicit Objects for Facelets and Programmatic Access

The following Implicit Objects for Facelets and Programmatic Access must be resolved using CDI,
except for environment-specific types _request_, _session_, and _application_, which must be
resolved via implicit EL resolvers. This separation maintains proper specification boundaries,
as the CDI specification mandates that servlet containers provide built-in beans for _@Inject_
support of _HttpServletRequest_, _HttpSession_, and _ServletContext_.

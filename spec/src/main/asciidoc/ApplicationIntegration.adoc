...
The _<f:metadata>_ section containing _<f:viewParam>_ elements must be processed regardless of whether it appears
within template composition tags such as _<ui:composition>_, _<ui:define>_, or _<ui:insert>_. The _ViewMetadata_
obtained via _ViewDeclarationLanguage.getViewMetadata()_ must reflect all view parameters defined in the
complete composed view, including those within template sections. Implementations must ensure that the
entire Facelets template hierarchy is considered when collecting view metadata, not just the top-level
template file.

...
basic_faces_flow_call/
  ...
  src/main/webapp/
             ...
             flow-b/
               ...
               next_a.xhtml
               next_b.xhtml
  src/main/java/com/example/basic_faces_flow_call/
                                FlowA.java
                                Flow_a_Bean.java
                                Flow_b_Bean.java
....
...
The _initView()_ method must be called immediately after checking for the
existence of the _FacesContext_ parameter and before invoking any
_PhaseListener.beforePhase()_ callbacks for the _Restore View Phase_. See the javadocs for this
...
also interact with the associated _StateManagementStrategy_, by calling
its _saveView()_ method, to ensure that state information for current
view is saved between requests.
...
obtain the view parameters corresponding to the argument _viewId_ and
...
** When navigating from one view to another (current _viewId_ differs from argument _viewId_):
The view parameter values must be obtained from the *current view's context* at the time the URL is generated.
Specifically, when executing the step "locate the _UIViewParameter_ instance in the current view whose name is
equivalent to the current element and let value be the result of calling _getStringValue()_ on the located
_UIViewParameter_", the implementation must use the _UIViewParameter_ instance from the current _UIViewRoot_
(the source page), not from the destination view. The destination view's backing beans must not be accessed
during this URL generation phase. This ensures that components like _<h:button>_ and _<h:link>_ with
_includeViewParams="true"_ correctly propagate the source page's parameter values to the destination URL.

** When using Facelets template composition:
The _<f:metadata>_ section containing _<f:viewParam>_ elements must only be defined in the template client
page (the root page that corresponds to the _viewId_, which uses _<ui:composition template="...">_), and
must not be defined in the template file itself (the file referenced by the _template_ attribute) nor in
any files included via _<ui:include>_. Within the template client page, the _<f:metadata>_ section may be
placed either inside _<ui:define>_ tags (to be inserted into a corresponding _<ui:insert>_ in the template)
or outside of _<ui:define>_ tags at the root level. If _<f:metadata>_ is placed in a template file or
included file, the implementation must throw a _TagException_. The _ViewMetadata_ obtained via
_ViewDeclarationLanguage.getViewMetadata()_ must reflect only the view parameters defined in the template
client page (the root page for the _viewId_).

* If argument _includeViewParams_ is _false_,
take no action to add additional entries to _parameters_. Let
...
* Call _saveView()_ on the
_StateManagementStrategy_ for this view, saving the result in a
thread-safe manner for use in the _writeState()_ method of _ViewHandler_.
...
associated _StateManagementStrategy_, passing the _FacesContext_ instance
for the current request and the calculated _viewId_, and return the
returned _UIViewRoot_, which may be _null_.
...
The state itself is collected by
_StateManagementStrategy.saveView()_, which returns it in a
_java.lang.Object_ instance that must implement
_java.io.Serializable_. If _null_ is returned from that method, there
is no state to save.
...
That method must also enforce the rule that
component ids within a _NamingContainer_ must be unique

==== State Restoring Methods

The component state of the view is restored by
_StateManagementStrategy.restoreView()_, in an implementation dependent
manner.

...
ExpressionFactory from theJakarta Server Pagescontainer by calling
...
application resource that produces the view, such as aJakarta Server Pagespage or a
Facelets page. In theJakarta Server Pagescase, this is a context relative path to the
...
rendering ofJakarta Server Pagespages containing Jakarta Faces components and Facelets pages
...
existingJakarta Server Pagesapplications, the concept of the _View Declaration
...
definition, bothJakarta Server Pagesand Facelets are examples of an implementation of a
...
instance for views written in eitherJakarta Server Pagesor Facelets.
[P1-end_required_ViewDeclarationLanguageImpls]Whether the instance
returned is the same for aJakarta Server Pagesor a Facelet view is an implementation
detail.
...
* Behavior required of the implementation that
handlesJakarta Server Pagesviews
...
TheJakarta Server Pagesand implementation is not required to
...
TheJakarta Server Pagesimplementation must return _null_.
...
{empty}TheJakarta Server Pagesimplementation is not required
to support argument _componentResource_ being aJakarta Server Pagesmarkup file. In this
...
{empty}TheJakarta Server Pagesimplementation is not required
to support argument _viewId_ being aJakarta Server Pagesmarkup file. In this case,
...
TheJakarta Server Pagesimplementation must:
...
* Execute theJakarta Server Pagespage to build the view by
...
the next step. This causes control to pass to theJakarta Server Pagescontainer, and
...
executing theJakarta Server Pagespage.
...
with Facelet based views, notJakarta Server Pagesbased views.

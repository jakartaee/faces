...
._Pass Through Attributes on Facets_

Pass through attributes may be specified on _<f:facet>_ tags using the
_jakarta.faces.passthrough_ XML namespace described above. When pass through
attributes are declared on a facet, the following behavior applies:

* If the _<f:facet>_ tag contains pass through attributes but no child _UIComponent_,
the runtime must automatically create a _UIComponent_ with component-family of
_jakarta.faces.Panel_ and renderer-type _jakarta.faces.Group_ as the facet component.
The pass through attributes must be associated with this automatically created component.

* When rendering a facet that has pass through attributes, the renderer must write
the pass through attributes to the parent component's rendered output at the point
where the facet is rendered.
...
<h:dataTable value="#{customers}" var="customer">
  <f:facet name="header" pt:styleClass="table-header" pt:role="banner">
    <h:outputText value="Customer List"/>
  </f:facet>
  <f:facet name="footer" pt:class="table-footer"/>
  ...
</h:dataTable>
----
...
* The _header_ facet contains both a child component and pass through attributes.
The _styleClass_ and _role_ attributes will be rendered on the table's header element.

* The _footer_ facet contains only pass through attributes without a child component.
A _UIPanel_ is automatically created as the facet component, and the _class_ attribute
will be rendered on the table's footer element.
...
[[a5633]]
===== Template text and whitespace

Template text is the character data in a
Facelet VDL file which is not part of a tag from a Facelet tag library.
Within the body of an element, the template text between two consecutive
tags from a Facelet tag library, between such a tag and the start or the
end of that body, or, if that body contains no such tag, the whole of
that body, forms a single _text run_. Literal markup which is not a tag
from a Facelet tag library is part of the text run in which it appears.

A text run which contains at least one
character that is not whitespace must be rendered. Where a text run
which must be rendered begins or ends with whitespace, at least one
whitespace character must be rendered at that boundary.

Subject to that boundary requirement, the
runtime may replace any sequence of whitespace characters in a text run
which must be rendered by a single whitespace character, or remove it
entirely.

A text run which consists solely of
whitespace characters must not be rendered.

The following is the rationale for these
requirements. It is not itself a requirement on the implementation.

The boundary requirement follows the way a
user agent treats character data in HTML: a run of whitespace collapses
to a single space, but it does not disappear. So the word _before_
followed by _<h:outputText value="component"/>_ renders as two words,
never as one.

The last requirement exists because a text
run becomes a child of the enclosing component, and renderers which lay
out their children positionally would otherwise receive the indentation
of the page as a child of its own. An _<h:panelGrid>_ with _columns="2"_
holding two children, written over separate indented lines, renders one
row of two cells rather than gaining cells for that indentation.

...
EnumConverter now autodiscovers type

* Issue ID {issue_tracker_url}1936[1936] +
Clarify UIViewRoot#resetValues behavior

* Issue ID {issue_tracker_url}1934[1934] +
Add FacesMessage.Severity enum and deprecate the string constants

* Issue ID {issue_tracker_url}1875[1875] +
Add missing UISelectItems#getVar/setVar methods
...
Unify ConfigurableNavigationHandler and NavigationHandler and deprecate ConfigurableNavigationHandler
...
Fix f:selectItems var attribute to support SelectItem and Map as well
...
Resolve _#{cc}_ and _#{component}_ using EL instead of CDI, and clarify expression language resolution on environment-specific types _request_, _session_, and _application_
...
Clarify that UIInput#isEmpty must be consulted during validate

* Issue ID {issue_tracker_url}1244[1244] +
Clarify how to compile EL in template text

* Issue ID {issue_tracker_url}1202[1202] +
Clarify cc:attributes type attribute
...
Clarify h:outputStyesheet request path rendering
...
Remove deprecated custom scope context
